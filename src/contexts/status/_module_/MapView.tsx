...
const renderChart = (node: HTMLElement | null, lineData: LineData, stopPoints: StopPoints) => {
  ...
  if (node) {
    ...
    lineData.coordinates.forEach(line => {
    ...
    const spLatLngs = stopPoints.map(
      sp => ({
        name: sp.name,
...
export default (props: Props) => {
  return (
    <div style={ { height: '100%' } } ref={ node => renderChart(node, props.lineData, props.stopPoints) }/>
  );
}
